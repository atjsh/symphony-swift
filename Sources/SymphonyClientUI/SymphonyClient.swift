...
extension URLSession: HTTPSessioning {
    func webSocketTask(with url: URL) -> any WebSocketTasking { URLSessionWebSocketTaskAdapter(task: webSocketTask(with: url)) }
}
...
private final class URLSessionWebSocketTaskAdapter: WebSocketTasking, @unchecked Sendable {
    ...
    func receive(completionHandler: @escaping @Sendable (Result<URLSessionWebSocketTask.Message, Error>) -> Void) { task.receive(completionHandler: completionHandler) }
...
public final class URLSessionSymphonyAPIClient: SymphonyAPIClientProtocol, @unchecked Sendable {
    ...
    public init(session: URLSession = .shared, decoder: JSONDecoder = JSONDecoder()) { self.session = session; self.decoder = decoder }

    init(session: any HTTPSessioning, decoder: JSONDecoder = JSONDecoder()) { self.session = session; self.decoder = decoder }

    public func health(endpoint: ServerEndpoint) async throws -> HealthResponse { try await request(endpoint: endpoint, method: "GET", path: "/api/v1/health") }

    public func issues(endpoint: ServerEndpoint) async throws -> IssuesResponse { try await request(endpoint: endpoint, method: "GET", path: "/api/v1/issues") }

    public func issueDetail(endpoint: ServerEndpoint, issueID: IssueID) async throws -> IssueDetail { try await request(endpoint: endpoint, method: "GET", path: "/api/v1/issues/\(issueID.rawValue)") }

    public func runDetail(endpoint: ServerEndpoint, runID: RunID) async throws -> RunDetail { try await request(endpoint: endpoint, method: "GET", path: "/api/v1/runs/\(runID.rawValue)") }

    public func logs(endpoint: ServerEndpoint, sessionID: SessionID, cursor: EventCursor?, limit: Int) async throws -> LogEntriesResponse {
        var queryItems = [URLQueryItem(name: "limit", value: String(limit))]
        if let cursor {
            queryItems.append(URLQueryItem(name: "cursor", value: cursor.rawValue))
        }
        return try await request(endpoint: endpoint, method: "GET", path: "/api/v1/logs/\(sessionID.rawValue)", queryItems: queryItems)
    }

    public func refresh(endpoint: ServerEndpoint) async throws -> RefreshResponse { try await request(endpoint: endpoint, method: "POST", path: "/api/v1/refresh") }

    public func logStream(endpoint: ServerEndpoint, sessionID: SessionID, cursor: EventCursor?) throws -> AsyncThrowingStream<AgentRawEvent, Error> {
    ...
    private func makeWebSocketURL(endpoint: ServerEndpoint, sessionID: SessionID, cursor: EventCursor?) throws -> URL {
        let scheme: String
