...
public final class SQLiteServerStateStore: @unchecked Sendable {
  ...
  private func stepRows<T>(_ statement: OpaquePointer, row: (OpaquePointer) throws -> T) throws
    -> [T]
  {
    var rows = [T]()
...
extension SQLiteServerStateStore {
  ...
  struct Diagnostics {
    ...
    func captureRuntimeErrorForRuntimeFailure() -> SymphonyRuntimeError? {
      captureRuntimeError { throw SymphonyRuntimeError.sqlite("Known diagnostic error.") }
    }

    func captureRuntimeErrorForUnexpectedProbe() -> SymphonyRuntimeError? {
      captureRuntimeError { throw ProbeError.rowFailure }
    }

    func captureRuntimeErrorForUnexpectedAutoclosureProbe() -> SymphonyRuntimeError? {
...
extension NSLock {
  fileprivate func sync<T>(_ body: () throws -> T) throws -> T {
    lock()
    defer { unlock() }
    return try body()
  }
