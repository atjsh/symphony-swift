...
public final class AgentRunner: AgentRunning, @unchecked Sendable {
  ...
  public func executeRun(
    ...
    do {
      ...
      RuntimeLogger.log(
        ...
        context: transitionContext(base: logContext, state: .preparingWorkspace),
        error: redactedLogMessage("Workspace preparation failed: \(error)")
      )
    ...
    do {
      ...
      RuntimeLogger.log(
        ...
        event: "agent_run_failed",
        context: transitionContext(base: logContext, state: .buildingPrompt),
        error: redactedLogMessage("Prompt render failed: \(error)")
      )
      let result = AgentRunResult(
        context: context, sessionID: sessionID, finalState: .failed,
    ...
    do {
      ...
      RuntimeLogger.log(
        ...
        event: "agent_run_failed",
        context: transitionContext(base: logContext, state: .initializingSession),
        error: redactedLogMessage("Session start failed: \(error)")
      )
      let result = AgentRunResult(
        context: context, sessionID: sessionID, finalState: .failed,
    ...
    do {
      for try await event in eventStream {
        ...
        if streamError == nil, adapter.providerName == .codex,
          let outcome = codexTurnOutcome(from: event.rawJSON)
        {
          switch outcome {
          case .completed:
            break
          case .failed:
            streamError = "Codex turn failed"
          case .interrupted:
            streamError = "Codex turn interrupted"
          }
        }
      }
    } catch {
    ...
    if streamError?.hasPrefix("Stall detected") == true {
      finalState = .stalled
    } else if isTimeoutError(terminalError) {
      finalState = .timedOut
    ...
    RuntimeLogger.log(
      ...
      error: streamError.map(redactedLogMessage)
    )
    return result
  }
  ...
  public func cancelRun(runID: RunID) async throws {
    ...
    if try await !activeRun.adapter.interruptSession(sessionID: activeRun.sessionID) {
...
private func isTimeoutError(_ error: Error?) -> Bool {
  guard let providerError = error as? ProviderAdapterError else { return false }
  switch providerError {
  case .readTimeout, .turnTimeout:
    return true
  default:
    return false
  }
