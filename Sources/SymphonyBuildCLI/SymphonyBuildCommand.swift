...
extension SymphonyBuildCommand {
  struct Build: ParsableCommand {
    static let configuration = CommandConfiguration(
      abstract: "Prepare Symphony build artifacts through xcodebuild.")
  ...
  struct Test: ParsableCommand {
    static let configuration = CommandConfiguration(
      abstract: "Prepare Symphony test artifacts through xcodebuild.")
  ...
  struct Run: ParsableCommand {
    static let configuration = CommandConfiguration(
      abstract: "Launch a local Symphony product for development.")
  ...
  struct Harness: ParsableCommand {
    static let configuration = CommandConfiguration(
      abstract: "Run the commit harness: package tests plus first-party source coverage gating.")
  ...
  struct Artifacts: ParsableCommand {
    static let configuration = CommandConfiguration(
      abstract: "Print stable inspection paths for the latest or selected run.")
...
extension SymphonyBuildCommand.Sim {
  struct List: ParsableCommand {
    static let configuration = CommandConfiguration(
      abstract: "List available simulator names and UDIDs.")

    mutating func run() throws {
      CLIContext.emit(
        try CLIContext.makeTool().simList(currentDirectory: CLIContext.currentDirectory()))
    }
  }
  ...
  struct Boot: ParsableCommand {
    ...
    mutating func run() throws {
      CLIContext.emit(
        try CLIContext.makeTool().simBoot(
          SimBootRequest(simulator: simulator, currentDirectory: CLIContext.currentDirectory())))
    }
  }

  struct SetServer: ParsableCommand {
    static let configuration = CommandConfiguration(
      abstract: "Record local endpoint values for future client launches.")
  ...
  struct ClearServer: ParsableCommand {
    static let configuration = CommandConfiguration(
      abstract: "Remove the persisted local endpoint override.")

    mutating func run() throws {
      CLIContext.emit(
        try CLIContext.makeTool().simClearServer(currentDirectory: CLIContext.currentDirectory()))
    }
  }
}
...
extension SymphonyBuildCommand.Hooks {
  struct Install: ParsableCommand {
    static let configuration = CommandConfiguration(
      abstract:
        "Configure Git to use the committed .githooks directory for this clone/worktree set.")

    mutating func run() throws {
      CLIContext.emit(
        try CLIContext.makeTool().hooksInstall(
          HooksInstallRequest(currentDirectory: CLIContext.currentDirectory())))
    }
  }
}
