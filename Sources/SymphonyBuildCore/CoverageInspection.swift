...
public struct CoverageInspectionFileCandidate: Hashable, Sendable {
    ...
    public init(targetName: String, path: String, coveredLines: Int, executableLines: Int, lineCoverage: Double) {
...
public struct SwiftPMCoverageInspector {
    ...
    public func resolveContext(coverageJSONPath: URL) throws -> SwiftPMCoverageContext {
        ...
        guard fileManager.fileExists(atPath: profileDataPath.path) else {
            throw SymphonyBuildError(
                ...
                message: "SwiftPM coverage inspection requires a profile data file at \(profileDataPath.path)."
            )
        }
        ...
        let preferredBinaryPath = debugRoot
        ...
        for candidate in candidates {
            ...
            if includeMissingLines {
                ...
                guard result.exitStatus == 0 else {
                    throw SymphonyBuildError(
                        ...
                        message: result.combinedOutput.isEmpty ? "Failed to inspect SwiftPM missing lines for \(candidate.path)." : result.combinedOutput
                    )
                }
                rawCommands.append(
                ...
                missingLineRanges = Self.parseAnnotatedMissingLineRanges(output: result.stdout, separator: "|")
            } else {
                missingLineRanges = []
            }
            ...
            if includeFunctions {
                ...
                guard result.exitStatus == 0 else {
                    throw SymphonyBuildError(
                        code: "swiftpm_coverage_inspection_failed",
                        message: result.combinedOutput.isEmpty ? "Failed to inspect SwiftPM functions for \(candidate.path)." : result.combinedOutput
                    )
                }
                rawCommands.append(
                    CoverageInspectionRawCommand(
    ...
    static func parseAnnotatedMissingLineRanges(output: String, separator: Character) -> [CoverageLineRange] {
    ...
    static func parseLLVMCovFunctions(output: String) -> [CoverageInspectionFunctionReport] {
        let regex = try? NSRegularExpression(
            pattern: #"^\s*(.*?)\s+\d+\s+\d+\s+[0-9.]+%\s+(\d+)\s+(\d+)\s+([0-9.]+)%\s+\d+\s+\d+\s+[0-9.]+%\s*$"#
        )
        ...
        return output
            ...
            .compactMap { rawLine -> CoverageInspectionFunctionReport? in
                ...
                guard !line.trimmingCharacters(in: .whitespaces).isEmpty,
                      ...
                      let lineCoverage = Double(line[coverageRange]) else {
                    return nil
                }
...
public struct XcodeCoverageInspector {
    ...
    public func inspect(
        ...
        for candidate in candidates {
            let missingLineRanges: [CoverageLineRange]
            if includeMissingLines {
                let commandLine = renderedMissingLinesCommandLine(resultBundlePath: resultBundlePath, filePath: candidate.path)
                let result = try processRunner.run(
                    command: "xcrun",
                    arguments: ["xccov", "view", "--archive", "--file", candidate.path, resultBundlePath.path],
                    environment: [:],
                    currentDirectory: nil,
                    observation: nil
                )
                guard result.exitStatus == 0 else {
                    throw SymphonyBuildError(
                        ...
                        message: result.combinedOutput.isEmpty ? "Failed to inspect Xcode missing lines for \(candidate.path)." : result.combinedOutput
                    )
                }
                rawCommands.append(
                    CoverageInspectionRawCommand(
                ...
                missingLineRanges = SwiftPMCoverageInspector.parseAnnotatedMissingLineRanges(output: result.stdout, separator: ":")
            } else {
                missingLineRanges = []
            }
            ...
            let functions: [CoverageInspectionFunctionReport]
            if includeFunctions {
                let commandLine = renderedFunctionsCommandLine(resultBundlePath: resultBundlePath, filePath: candidate.path)
                let result = try processRunner.run(
                    command: "xcrun",
                    arguments: ["xccov", "view", "--report", "--functions-for-file", candidate.path, resultBundlePath.path],
                    environment: [:],
                    currentDirectory: nil,
                    observation: nil
                )
                guard result.exitStatus == 0 else {
                    throw SymphonyBuildError(
                        ...
                        message: result.combinedOutput.isEmpty ? "Failed to inspect Xcode functions for \(candidate.path)." : result.combinedOutput
                    )
                }
                rawCommands.append(
                    CoverageInspectionRawCommand(
    ...
    func renderedMissingLinesCommandLine(resultBundlePath: URL, filePath: String) -> String {
        ShellQuoting.render(command: "xcrun", arguments: ["xccov", "view", "--archive", "--file", filePath, resultBundlePath.path])
    }

    func renderedFunctionsCommandLine(resultBundlePath: URL, filePath: String) -> String {
        ShellQuoting.render(command: "xcrun", arguments: ["xccov", "view", "--report", "--functions-for-file", filePath, resultBundlePath.path])
    }
