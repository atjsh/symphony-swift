  ...
  static func runCoverageSuiteExecution(
    ...
    guard !result.stdout.isEmpty else {
      throw SymphonyBuildError(
        ...
        message: "The coverage command did not emit JSON output.")
    }
    ...
    do {
      ...
      return CoverageSuiteExecution(
        report: try JSONDecoder().decode(CoverageReport.self, from: data), inspection: nil)
    } catch {
      throw SymphonyBuildError(
        code: "bootstrap_coverage_decode_failed",
        message: "The coverage command JSON output could not be decoded.")
    }
  }
