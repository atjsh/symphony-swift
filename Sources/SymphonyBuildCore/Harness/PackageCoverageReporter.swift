...
public struct PackageCoverageReporter {
  ...
  public func loadReport(at coverageJSONPath: URL, projectRoot: URL) throws -> PackageCoverageReport
  {
    guard fileManager.fileExists(atPath: coverageJSONPath.path) else {
      throw SymphonyBuildError(
        code: "missing_package_coverage_json",
    ...
    do {
      ...
      throw SymphonyBuildError(
        code: "package_coverage_decode_failed",
        message: "SwiftPM coverage JSON could not be decoded.")
    }

    let sourcesRoot =
      projectRoot.resolvingSymlinksInPath().appendingPathComponent("Sources", isDirectory: true)
      .path + "/"
    let files = export.data
      ...
      .compactMap { file -> PackageCoverageFileReport? in
        ...
        let relativePath = String(
          file.filename.dropFirst(projectRoot.resolvingSymlinksInPath().path.count + 1))
        let coveredLines = file.summary.lines.covered
        return PackageCoverageFileReport(
          path: relativePath,
          coveredLines: coveredLines,
          executableLines: executableLines,
          lineCoverage: Self.normalizedCoverage(
    ...
    guard !files.isEmpty else {
      throw SymphonyBuildError(
        code: "package_coverage_sources_missing",
        message: "SwiftPM coverage did not include any first-party files under Sources/.")
    }
  ...
  public func renderHuman(report: HarnessReport) -> String {
    let threshold = String(
      format: "%.2f%%", locale: Locale(identifier: "en_US_POSIX"), report.minimumCoveragePercent)
    var lines = [
      "tests passed",
    ...
    if !report.packageCoverage.files.isEmpty {
      ...
      for file in report.packageCoverage.files.prefix(10) {
        lines.append(
          "file \(file.path) \(percentage(file.lineCoverage)) (\(file.coveredLines)/\(file.executableLines))"
        )
      }
    }
    ...
    if let clientCoverage = report.clientCoverage {
      ...
      for target in clientCoverage.targets {
        lines.append(
          "target \(target.name) \(percentage(target.lineCoverage)) (\(target.coveredLines)/\(target.executableLines))"
        )
      }
    }
    ...
    if !report.violations.isEmpty {
      ...
      for violation in report.violations.sorted(by: violationSort) {
        ...
        if let missingLineRanges = violation.missingLineRanges, !missingLineRanges.isEmpty {
          lines.append("  missing_lines \(renderMissingLineRanges(missingLineRanges))")
        }
  ...
  public func makePackageFileViolations(report: PackageCoverageReport, minimumLineCoverage: Double)
  ...
  public func makeTargetViolations(
    report: CoverageReport, suite: String, minimumLineCoverage: Double
  ) -> [HarnessCoverageViolation] {
  ...
  public func makeFileViolations(report: CoverageReport, suite: String, minimumLineCoverage: Double)
    -> [HarnessCoverageViolation]
  {
  ...
  static func packageFileSort(lhs: PackageCoverageFileReport, rhs: PackageCoverageFileReport)
    -> Bool
  {
