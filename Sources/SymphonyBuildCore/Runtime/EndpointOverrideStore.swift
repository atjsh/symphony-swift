...
public struct EndpointOverrideStore {
    ...
    public func resolve(
        ...
        if let serverURL {
            guard let components = URLComponents(string: serverURL),
                  ...
                  let resolvedPort = components.port else {
                throw SymphonyBuildError(code: "invalid_server_url", message: "The provided server URL must include a scheme, host, and port.")
            }
    ...
    public func storeURL(in workspace: WorkspaceContext) -> URL {
        workspace.buildStateRoot.appendingPathComponent("runtime/server-endpoint.json", isDirectory: false)
    }
}
