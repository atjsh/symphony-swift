...
public struct ProductDetails: Sendable {
    ...
    public var productURL: URL {
        targetBuildDirectory.appendingPathComponent(fullProductName, isDirectory: fullProductName.hasSuffix(".app"))
    }
}
...
public struct ProductLocator {
    ...
    public func locateProduct(
        ...
        var arguments = ["-showBuildSettings", "-json", "-scheme", scheme, "-destination", destination.xcodeDestination, "-derivedDataPath", derivedDataPath.path]
        ...
        guard result.exitStatus == 0 else {
            throw SymphonyBuildError(code: "show_build_settings_failed", message: result.combinedOutput.isEmpty ? "Failed to query build settings." : result.combinedOutput)
        }

        let decoded = try JSONDecoder().decode([BuildSettingsContainer].self, from: Data(result.stdout.utf8))
        guard let settings = decoded.first?.buildSettings else {
            throw SymphonyBuildError(code: "missing_build_settings", message: "xcodebuild did not return build settings for the selected scheme.")
        }

        guard let targetBuildDirectory = settings["TARGET_BUILD_DIR"]?.stringValue,
              let fullProductName = settings["FULL_PRODUCT_NAME"]?.stringValue else {
            throw SymphonyBuildError(code: "incomplete_build_settings", message: "xcodebuild returned incomplete product settings.")
        }
