...
public final class SymphonyBuildTool {
  ...
  public func coverage(_ request: CoverageCommandRequest) throws -> String {
    ...
    let selector = SchemeSelector(
      product: request.product, scheme: request.scheme, platform: request.platform)
    let executionContext = try executionContextBuilder.make(
      workspace: workspace, worker: worker, command: .coverage, runID: selector.runIdentifier)
    switch selector.product.defaultBackend {
    case .xcode:
      if !request.dryRun {
        try ensureXcodeSupport(for: selector.platform)
  ...
  private func coverageXcode(
    ...
    let record = try artifactManager.recordXcodeExecution(
      ...
      exitStatus: result.exitStatus == 0 && coverageAnomalies.isEmpty
        ? 0 : (result.exitStatus == 0 ? 1 : result.exitStatus),
      combinedOutput: result.combinedOutput,
      startedAt: startedAt,
      endedAt: endedAt,
    ...
    guard result.exitStatus == 0 else {
      throw SymphonyBuildCommandFailure(
        message: "xcodebuild test with code coverage failed.", summaryPath: record.run.summaryPath)
    }
    guard coverageAnomalies.isEmpty, let coverageArtifacts else {
      throw SymphonyBuildCommandFailure(
        message: "Coverage export failed.", summaryPath: record.run.summaryPath)
    }
  ...
  private func coverageSwiftPM(
    ...
    guard result.exitStatus == 0 else {
      throw SymphonyBuildCommandFailure(
        message: "swift test with code coverage failed.", summaryPath: record.run.summaryPath)
    }
    guard coverageAnomalies.isEmpty, let coverageArtifacts else {
      throw SymphonyBuildCommandFailure(
        message: "Coverage export failed.", summaryPath: record.run.summaryPath)
  ...
  private func runXcode(
    ...
    let endpoint = try endpointOverrideStore.resolve(
      workspace: workspace, serverURL: request.serverURL, host: request.host, port: request.port)
    ...
    if request.json {
      ...
      return try encodePrettyJSON(
        CoverageInspectionResponse(
          coverage: coverageArtifacts.report, inspection: inspectionPayload)
      )
    }
