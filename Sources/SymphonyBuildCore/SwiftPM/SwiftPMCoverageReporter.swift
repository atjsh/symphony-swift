...
public struct SwiftPMCoverageReporter {
    ...
    public func renderedCoveragePathCommandLine() -> String { ShellQuoting.render(command: "swift", arguments: ["test", "--show-code-coverage-path"]) }
    ...
    public func exportServerCoverage(
        ...
        do {
            ...
            throw SymphonyBuildError(code: "swiftpm_coverage_decode_failed", message: "SwiftPM coverage JSON could not be decoded.")
        }

        let resolvedRoot = projectRoot.resolvingSymlinksInPath()
        let targetRoots = [
            ("SymphonyRuntime", resolvedRoot.appendingPathComponent("Sources/SymphonyRuntime", isDirectory: true).path + "/"),
            ("SymphonyServer", resolvedRoot.appendingPathComponent("Sources/SymphonyServer", isDirectory: true).path + "/"),
        ]
        ...
        for file in export.data.flatMap(\.files) {
            ...
            targetFiles.append(
                CoverageFileReport(
                    ...
                    lineCoverage: CoverageReporter.normalizedCoverage(coveredLines: coveredLines, executableLines: executableLines)
                )
            )
        ...
        let targets = targetRoots.compactMap { targetName, _ -> CoverageTargetReport? in
            ...
            let coveredLines = files.reduce(0) { $0 + $1.coveredLines }, executableLines = files.reduce(0) { $0 + $1.executableLines }
        ...
        guard !targets.isEmpty else {
            throw SymphonyBuildError(
                ...
                message: "SwiftPM coverage did not include any first-party server files under Sources/SymphonyRuntime or Sources/SymphonyServer."
            )
        }

        let coveredLines = targets.reduce(0) { $0 + $1.coveredLines }, executableLines = targets.reduce(0) { $0 + $1.executableLines }
        ...
        let jsonPath = artifactRoot.appendingPathComponent("coverage.json"), textPath = artifactRoot.appendingPathComponent("coverage.txt")
