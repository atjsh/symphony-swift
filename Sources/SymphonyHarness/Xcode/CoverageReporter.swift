...
public struct CoverageArtifacts: Sendable {
  ...
  public init(
    report: CoverageReport, jsonPath: URL, textPath: URL, jsonOutput: String, textOutput: String
  ) {
    self.report = report
...
public struct CoverageReporter {
  ...
  public init(
    processRunner: ProcessRunning = SystemProcessRunner(), fileManager: FileManager = .default
  ) {
  ...
  public func export(
    ...
    guard result.exitStatus == 0, !result.stdout.isEmpty else {
      throw SymphonyHarnessError(
        ...
        message: result.combinedOutput.isEmpty
          ? "Failed to export coverage from the xcresult bundle." : result.combinedOutput
      )
    }
    ...
    do {
      ...
      throw SymphonyHarnessError(
        code: "coverage_report_decode_failed",
        message: "The xccov JSON output could not be decoded.")
    }
    ...
    let targets = includedTargets.map { target in
      CoverageTargetReport(
        ...
        lineCoverage: Self.normalizedCoverage(
          coveredLines: target.coveredLines, executableLines: target.executableLines),
        files: showFiles
          ? target.files.map { file in
            CoverageFileReport(
              name: file.name,
              path: file.path,
              ...
              lineCoverage: Self.normalizedCoverage(
                coveredLines: file.coveredLines, executableLines: file.executableLines)
            )
          } : nil
      )
    }
  ...
  func renderHuman(report: CoverageReport) -> String {
    ...
    for target in report.targets {
      ...
      if let files = target.files {
        for file in files {
          lines.append(
            "file \(target.name) \(file.name) \(percentage(file.lineCoverage)) (\(file.coveredLines)/\(file.executableLines))"
          )
        }
      }
    }
  ...
  static func normalizedCoverage(coveredLines: Int, executableLines: Int) -> Double {
    executableLines > 0 ? Double(coveredLines) / Double(executableLines) : 0
  }
