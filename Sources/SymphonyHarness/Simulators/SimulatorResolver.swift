...
public struct SimctlSimulatorCatalog: SimulatorCataloging {
  ...
  public func availableDevices() throws -> [SimulatorDevice] {
    let result = try processRunner.run(
      command: "xcrun", arguments: ["simctl", "list", "devices", "available", "-j"],
      environment: [:], currentDirectory: nil)
    guard result.exitStatus == 0 else {
      throw SymphonyHarnessError(
        code: "simctl_failed",
        message: result.combinedOutput.isEmpty
          ? "Failed to query available simulators." : result.combinedOutput)
    }
  ...
  private static func devicesForRuntime(runtime: String, devices: [SimctlDevice])
    -> [SimulatorDevice]
  {
...
public struct SimulatorResolver {
  ...
  public init(
    catalog: SimulatorCataloging = SimctlSimulatorCatalog(),
    ...
    case .iosSimulator:
      ...
      if fuzzyMatches.count > 1 {
        throw SymphonyHarnessError(
          ...
          message:
            "Simulator search '\(requested)' matched multiple devices. Use a more specific name or a UDID."
        )
      }
  ...
  public func approvedValidationDestinations() throws -> [ResolvedDestination] {
    let devices = try catalog.availableDevices()

    guard let iPhone = devices.first(where: { $0.name.localizedCaseInsensitiveContains("iphone") }) else {
      throw SymphonyHarnessError(
        code: "missing_iphone_validation_destination",
        message: "No approved iPhone simulator is available for default app validation."
      )
    }

    guard let iPad = devices.first(where: { $0.name.localizedCaseInsensitiveContains("ipad") }) else {
      throw SymphonyHarnessError(
        code: "missing_ipad_validation_destination",
        message: "No approved iPad simulator is available for default app validation."
      )
    }
    ...
    return [
      ResolvedDestination(
        platform: .iosSimulator,
        displayName: "\(iPhone.name) (\(iPhone.udid))",
        simulatorName: iPhone.name,
        simulatorUDID: iPhone.udid,
        xcodeDestination: "platform=iOS Simulator,id=\(iPhone.udid)"
      ),
      ResolvedDestination(
        platform: .iosSimulator,
        displayName: "\(iPad.name) (\(iPad.udid))",
        simulatorName: iPad.name,
        simulatorUDID: iPad.udid,
        xcodeDestination: "platform=iOS Simulator,id=\(iPad.udid)"
      ),
    ]
  }
  ...
  public func boot(resolved destination: ResolvedDestination) throws {
    ...
    let result = try processRunner.run(
      command: "xcrun", arguments: ["simctl", "bootstatus", udid, "-b"], environment: [:],
    ...
    let boot = try processRunner.run(
      command: "xcrun", arguments: ["simctl", "boot", udid], environment: [:], currentDirectory: nil
    )
    if boot.exitStatus != 0
      && !boot.combinedOutput.contains("Unable to boot device in current state")
    {
      throw SymphonyHarnessError(
        code: "simulator_boot_failed",
        message: boot.combinedOutput.isEmpty
          ? "Failed to boot simulator \(udid)." : boot.combinedOutput)
    }

    let ready = try processRunner.run(
      command: "xcrun", arguments: ["simctl", "bootstatus", udid, "-b"], environment: [:],
      currentDirectory: nil)
    guard ready.exitStatus == 0 else {
      throw SymphonyHarnessError(
        code: "simulator_boot_failed",
        message: ready.combinedOutput.isEmpty
          ? "Failed to confirm simulator boot." : ready.combinedOutput)
    }
  }
}

extension SimulatorResolver {
  fileprivate static var hostMacOSDestination: String {
    #if arch(arm64)
