...
`Symphony Server`, a cross-platform `Symphony` SwiftUI client, and the
...
1. `Symphony Server`
   ...
   - Owns the SQLite-backed runtime state, HTTP query API, WebSocket log stream, refresh
...
The specification describes product behavior and contracts. The server requires Hummingbird-backed
...
- Support restart recovery for historical state and observability without requiring active session
  resumption.
- Poll GitHub for active issues, orchestrate dispatch, manage workspaces, and run provider agents.
...
7. The API exposes current state, historical state, and log replay/streaming.
8. `Symphony` fetches baseline data over HTTP and uses WebSocket for live log tails.
...
- The orchestrator polls GitHub, dispatches to workspaces, runs providers, and reconciles.
...
- Hummingbird is the normative server framework, and custom HTTP parsing is not a target
  architecture.
