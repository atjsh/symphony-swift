...
@Test func commitHarnessUsesDefaultCoverageLoadersForBothClientAndServer() throws {
  try withTemporaryDirectory { directory in
    ...
    let runner = DualCoverageProcessRunner(
      packageCoveragePath: coveragePath.path, coverageJSON: coverageJSON)
...
@Test func commitHarnessExecuteDecodesInspectionWrappersAndRequestsInspectionFlags() throws {
  try withTemporaryDirectory { directory in
    ...
    let runner = RecordingCoverageInspectionProcessRunner(
      packageCoveragePath: coveragePath.path, wrappedCoverageJSON: wrapperJSON)
    ...
    #expect(
      runner.commands.contains(where: {
        $0.contains("--show-functions") && $0.contains("--show-missing-lines")
      }))
    ...
    let rawWrapperJSON = String(
      decoding: try JSONEncoder().encode(
        CoverageInspectionResponse(coverage: coverageReport, inspection: .raw(rawInspection))),
      as: UTF8.self
    )
    let rawRunner = RecordingCoverageInspectionProcessRunner(
      packageCoveragePath: coveragePath.path, wrappedCoverageJSON: rawWrapperJSON)
  ...
  try withTemporaryDirectory { directory in
    ...
    #expect(
      quietRunner.commands.contains(where: {
        $0.contains("coverage --product client") && $0.contains("--xcode-output-mode quiet")
      }))
    #expect(
      quietRunner.commands.contains(where: {
        $0.contains("coverage --product server") && $0.contains("--xcode-output-mode quiet")
      }))
  }
}
...
private struct CoverageCommandProcessRunner: ProcessRunning {
  ...
  func run(
    ...
    if arguments.prefix(3) == ["coverage", "--product", "client"]
      || arguments.prefix(3) == ["coverage", "--product", "server"]
    {
