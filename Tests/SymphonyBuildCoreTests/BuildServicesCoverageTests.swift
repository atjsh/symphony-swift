...
@Test func productLocatorCoversSuccessAndFailureModes() throws {
  ...
  let workspaceCommand =
    "xcodebuild -showBuildSettings -json -scheme Symphony -destination \(expectedHostMacOSDestination()) -derivedDataPath \(derivedDataPath.path) -workspace /tmp/repo/Symphony.xcworkspace"
  ...
  let projectCommand =
    "xcodebuild -showBuildSettings -json -scheme Symphony -destination \(expectedHostMacOSDestination()) -derivedDataPath \(derivedDataPath.path) -project /tmp/repo/SymphonyApps.xcodeproj"
  ...
  do {
    _ = try ProductLocator(
      ...
      workspace: workspace, scheme: "Symphony", destination: destination,
      derivedDataPath: derivedDataPath)
  ...
  try withTemporaryDirectory { directory in
    ...
    #expect(
      store.storeURL(in: endpointWorkspace).path.hasSuffix(
        ".build/symphony-build/runtime/server-endpoint.json"))
    #expect(
    ...
    let buildStateFile = repoRoot.appendingPathComponent(
      ".build/symphony-build", isDirectory: false)
    try FileManager.default.createDirectory(
      ...
      workspaceDiscovery: StubWorkspaceDiscovery(
        workspace: WorkspaceContext(
          ...
          buildStateRoot: repoRoot.appendingPathComponent(
            ".build/symphony-build", isDirectory: true),
