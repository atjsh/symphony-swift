...
@Test func coverageCommandParsesInspectionFlags() throws {
  let command =
    try SymphonyBuildCommand.Coverage.parseAsRoot([
      "--show-files",
      "--show-functions",
      "--show-missing-lines",
      "--raw-output",
...
@Test func coverageCommandRejectsRawOutputWithoutInspectionSelection() throws {
  do {
    _ =
      try SymphonyBuildCommand.Coverage.parseAsRoot(["--raw-output"])
      as! SymphonyBuildCommand.Coverage
...
@Test func harnessCommandParsesOutputMode() throws {
  let command =
    try SymphonyBuildCommand.Harness.parseAsRoot(["--output-mode", "quiet"])
    as! SymphonyBuildCommand.Harness
...
@Test func commandsParseExplicitXcodeOutputModes() throws {
  let build =
    try SymphonyBuildCommand.Build.parseAsRoot(["--xcode-output-mode", "full"])
    as! SymphonyBuildCommand.Build
  let test =
    try SymphonyBuildCommand.Test.parseAsRoot(["--xcode-output-mode", "quiet"])
    as! SymphonyBuildCommand.Test
  let coverage =
    try SymphonyBuildCommand.Coverage.parseAsRoot(["--xcode-output-mode", "full"])
    as! SymphonyBuildCommand.Coverage
  let run =
    try SymphonyBuildCommand.Run.parseAsRoot(["--xcode-output-mode", "full"])
    as! SymphonyBuildCommand.Run
...
@Test func hooksInstallCommandParses() throws {
  let command =
    try SymphonyBuildCommand.Hooks.Install.parseAsRoot([]) as! SymphonyBuildCommand.Hooks.Install
...
@Test func runCommandDefaultsToServerAndCapturesEnvOverrides() throws {
  let command =
    try SymphonyBuildCommand.Run.parseAsRoot(["--env", "FOO=bar", "--env", "BAR=baz"])
    as! SymphonyBuildCommand.Run
...
@Test func simCommandsParseBootSetServerAndClearServerOptions() throws {
  ...
  let boot =
    try SymphonyBuildCommand.Sim.Boot.parseAsRoot(["--simulator", "iPhone 17 Pro"])
    as! SymphonyBuildCommand.Sim.Boot
  let setServer =
    try SymphonyBuildCommand.Sim.SetServer.parseAsRoot([
      "--scheme", "https",
      "--host", "api.example.com",
      "--port", "9443",
    ]) as! SymphonyBuildCommand.Sim.SetServer
  let clearServer =
    try SymphonyBuildCommand.Sim.ClearServer.parseAsRoot([])
    as! SymphonyBuildCommand.Sim.ClearServer
...
@Test func artifactsCommandDefaultsToBuildFamily() throws {
  let command =
    try SymphonyBuildCommand.Artifacts.parseAsRoot([]) as! SymphonyBuildCommand.Artifacts
...
@Test func artifactsCommandAllowsExplicitRunSelection() throws {
  let command =
    try SymphonyBuildCommand.Artifacts.parseAsRoot([
      "coverage", "--run", "20260324-120000-symphony",
    ]) as! SymphonyBuildCommand.Artifacts
...
@Test func artifactsCommandSupportsHarnessFamily() throws {
  let command =
    try SymphonyBuildCommand.Artifacts.parseAsRoot(["harness"]) as! SymphonyBuildCommand.Artifacts
...
@Test func commandRunsDispatchRequestsThroughCLIContext() throws {
  ...
  try CLIContext.withOverrides(
    ...
    var build =
      try SymphonyBuildCommand.Build.parseAsRoot([
        "--product", "server", "--dry-run", "--worker", "7",
      ]) as! SymphonyBuildCommand.Build
    try build.run()

    var test =
      try SymphonyBuildCommand.Test.parseAsRoot([
        "--product", "server", "--only-testing", "Suite/testThing",
      ]) as! SymphonyBuildCommand.Test
    try test.run()
    ...
    var run =
      try SymphonyBuildCommand.Run.parseAsRoot([
        "--env", "FOO=bar", "--host", "example.com", "--port", "9443",
      ]) as! SymphonyBuildCommand.Run
    try run.run()

    var harness =
      try SymphonyBuildCommand.Harness.parseAsRoot(["--minimum-coverage", "88", "--json"])
      as! SymphonyBuildCommand.Harness
    try harness.run()
    ...
    var boot =
      try SymphonyBuildCommand.Sim.Boot.parseAsRoot(["--simulator", "AAAA-BBBB"])
      as! SymphonyBuildCommand.Sim.Boot
    try boot.run()

    var setServer =
      try SymphonyBuildCommand.Sim.SetServer.parseAsRoot(["--server-url", "http://localhost:9000"])
      as! SymphonyBuildCommand.Sim.SetServer
    try setServer.run()
    ...
    var artifacts =
      try SymphonyBuildCommand.Artifacts.parseAsRoot(["harness", "--run", "run-1"])
      as! SymphonyBuildCommand.Artifacts
    try artifacts.run()

    var doctor =
      try SymphonyBuildCommand.Doctor.parseAsRoot(["--strict", "--json", "--quiet"])
      as! SymphonyBuildCommand.Doctor
    try doctor.run()
  }
...
@Test func runCommandRejectsInvalidEnvironmentOverrideFormat() throws {
  ...
  do {
    try CLIContext.withOverrides(
      ...
      var run =
        try SymphonyBuildCommand.Run.parseAsRoot(["--env", "BROKEN"]) as! SymphonyBuildCommand.Run
      try run.run()
    }
