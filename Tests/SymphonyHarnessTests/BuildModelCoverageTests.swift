...
@Test func buildModelsCoverRuntimeDiagnosticsRequestsAndUtilities() throws {
  ...
  let coverageFile = CoverageFileReport(
    name: "Foo.swift", path: "/tmp/Foo.swift", coveredLines: 10, executableLines: 10,
    lineCoverage: 1)
  let coverageTarget = CoverageTargetReport(
    name: "Symphony", buildProductPath: "/tmp/Symphony.app", coveredLines: 10, executableLines: 10,
    lineCoverage: 1, files: [coverageFile])
  let coverageReport = CoverageReport(
    coveredLines: 10, executableLines: 10, lineCoverage: 1, includeTestTargets: false,
    excludedTargets: ["SymphonySwiftUIAppTests.xctest"], targets: [coverageTarget])
  #expect(coverageReport.targets.first?.files?.first == coverageFile)
  let inspectionFunction = CoverageInspectionFunctionReport(
    name: "Foo.bar()", coveredLines: 2, executableLines: 3, lineCoverage: 2.0 / 3.0)
  ...
  let packageReport = PackageCoverageReport(
    scope: "first_party_sources", coveredLines: 5, executableLines: 5, lineCoverage: 1,
    coverageJSONPath: "/tmp/package.json", files: [packageFile])
  let violation = HarnessCoverageViolation(
    suite: "client", kind: "file", name: "/tmp/Foo.swift", coveredLines: 4, executableLines: 5,
    lineCoverage: 0.8)
  let harness = HarnessReport(
    ...
    serverCoverageInvocation: "harness test SymphonyServer --json",
  ...
  let anomaly = ArtifactAnomaly(
    code: "missing_recording", message: "No recording.", phase: "xcresult")
  let entry = ArtifactIndexEntry(
    name: "recording.mp4", relativePath: "recording.mp4", kind: "missing",
    createdAt: "2026-03-24T00:00:00Z", anomaly: anomaly)
  let index = ArtifactIndex(
    entries: [entry], command: .run, runID: artifactRun.runID, timestamp: artifactRun.timestamp,
    anomalies: [anomaly])
  ...
  let invalidRuntime = try JSONDecoder().decode(
    RuntimeEndpoint.self, from: Data(#"{"scheme":"","host":"","port":0}"#.utf8))
  ...
  #expect(
    DiagnosticsReport(
      issues: [DiagnosticIssue(severity: .warning, code: "warn", message: "warn")], notes: ["note"],
  ...
  #expect(
    try JSONDecoder().decode(
      HarnessCoverageInspectionArtifact.self, from: JSONEncoder().encode(skippedInspection))
      == skippedInspection)
  ...
  let buildRequest = BuildCommandRequest(
    product: .server, scheme: "Server", platform: .macos, simulator: nil, workerID: 1, dryRun: true,
    buildForTesting: true, outputMode: .quiet, currentDirectory: URL(fileURLWithPath: "/tmp"))
  let testRequest = TestCommandRequest(
    product: .client, scheme: "Client", platform: .iosSimulator, simulator: "UDID", workerID: 2,
    dryRun: false, onlyTesting: ["Suite/test"], skipTesting: ["Suite/skip"], outputMode: .filtered,
    currentDirectory: URL(fileURLWithPath: "/tmp"))
  let runRequest = RunCommandRequest(
    product: .server, scheme: "Server", platform: .macos, simulator: nil, workerID: 3,
    dryRun: false, serverURL: "http://localhost:8080", serverScheme: "http", host: nil, port: nil,
  ...
  let artifactsRequest = ArtifactsCommandRequest(
    command: .harness, latest: false, runID: "run-1", currentDirectory: URL(fileURLWithPath: "/tmp")
  )
  ...
  let simSetRequest = SimSetServerRequest(
    ...
    currentDirectory: URL(fileURLWithPath: "/tmp"))
  let simBootRequest = SimBootRequest(
    simulator: "AAAA-BBBB", currentDirectory: URL(fileURLWithPath: "/tmp"))
  #expect(buildRequest.buildForTesting)
  #expect(testRequest.skipTesting == ["Suite/skip"])
  #expect(runRequest.serverScheme == "http")
