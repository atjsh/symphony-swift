...
@Test func bootstrapServerRunnerRunPropagatesStartupFailuresAndSignalOnlyFiresOnce() async throws {
  ...
  let occupiedSocket = try bootstrapMakeListeningSocket(port: try bootstrapAvailableLoopbackPort())
  defer { close(occupiedSocket) }
  let occupiedPort = try bootstrapListeningPort(for: occupiedSocket)
  let root = try bootstrapMakeTemporaryDirectory()
...
@Test func serverStartupSignalSyncWaitCoversBlockingAndImmediateBranches() throws {
  ...
  let syncWaitError = BootstrapLockedErrorBox()
  let worker = Thread {
...
@Test func bootstrapEnvironmentWorkflowURLPrefersExplicitPath() throws {
  let explicitWorkflow = try bootstrapMakeTemporaryDirectory().appendingPathComponent("custom-workflow.md")
...
@Test func bootstrapEnvironmentRequiredWorkflowURLUsesExplicitEnvironmentPath() throws {
  let explicitWorkflow = try bootstrapMakeTemporaryDirectory().appendingPathComponent("required-workflow.md")
  try "---\nagent:\n  default_provider: codex\n---\nResolve it".write(
    to: explicitWorkflow,
    atomically: true,
    encoding: .utf8
