...
@Test func sqliteStorePersistsProviderNeutralStateAndReplaysLogsByCursor() throws {
    ...
    let loadedFirstPage = try reopened.logs(sessionID: fixture.session.sessionID, cursor: nil, limit: 1)
    ...
    let loadedSecondPage = try reopened.logs(sessionID: fixture.session.sessionID, cursor: firstPage.nextCursor, limit: 1)
...
@Test func sqliteStoreInitializationFailureAndMissingEntityBranchesAreReported() throws {
    ...
    do {
        ...
        #expect(String(describing: error).contains("Failed to open SQLite database at \(missingParentURL.path)."))
    }
    ...
    let defaultHealth = try defaultAPI.respond(to: SymphonyAPIRequest(method: "GET", path: "/api/v1/health"))
...
@Test func apiRouterServesSpecEndpointsAndUsesErrorEnvelope() throws {
    ...
    let healthResponse = try api.respond(to: SymphonyAPIRequest(method: "GET", path: "/api/v1/health"))
    #expect(healthResponse.statusCode == 200)
    let health = try decodeBody(HealthResponse.self, from: healthResponse)
    #expect(health == HealthResponse(status: "ok", serverTime: "2024-03-24T12:00:00Z", version: "1.0.0", trackerKind: "github"))

    let issuesResponse = try api.respond(to: SymphonyAPIRequest(method: "GET", path: "/api/v1/issues"))
    ...
    let issueDetailResponse = try api.respond(to: SymphonyAPIRequest(method: "GET", path: "/api/v1/issues/\(fixture.issue.id.rawValue)"))
    ...
    let runDetailResponse = try api.respond(to: SymphonyAPIRequest(method: "GET", path: "/api/v1/runs/\(fixture.runDetail.runID.rawValue)"))
    ...
    let logsResponse = try api.respond(to: SymphonyAPIRequest(method: "GET", path: "/api/v1/logs/\(fixture.session.sessionID.rawValue)?limit=50"))
    ...
    let refreshResponse = try api.respond(to: SymphonyAPIRequest(method: "POST", path: "/api/v1/refresh"))
    ...
    let missingIssueResponse = try api.respond(to: SymphonyAPIRequest(method: "GET", path: "/api/v1/issues/missing"))
    ...
    let unsupportedResponse = try api.respond(to: SymphonyAPIRequest(method: "DELETE", path: "/api/v1/issues"))
...
@Test func apiRouterCoversDefaultClockRefreshAndAdditionalErrorBranches() throws {
    ...
    let issueMethod = try api.respond(to: SymphonyAPIRequest(method: "POST", path: "/api/v1/issues/\(fixture.issue.id.rawValue)"))
    #expect(issueMethod.statusCode == 405)
    #expect(try decodeBody(ErrorEnvelope.self, from: issueMethod).error.code == "method_not_allowed")

    let runMethod = try api.respond(to: SymphonyAPIRequest(method: "POST", path: "/api/v1/runs/\(fixture.runDetail.runID.rawValue)"))
    #expect(runMethod.statusCode == 405)
    #expect(try decodeBody(ErrorEnvelope.self, from: runMethod).error.code == "method_not_allowed")

    let missingRun = try api.respond(to: SymphonyAPIRequest(method: "GET", path: "/api/v1/runs/missing-run"))
    #expect(missingRun.statusCode == 404)
    #expect(try decodeBody(ErrorEnvelope.self, from: missingRun).error.code == "run_not_found")

    let logsMethod = try api.respond(to: SymphonyAPIRequest(method: "POST", path: "/api/v1/logs/\(fixture.session.sessionID.rawValue)"))
    #expect(logsMethod.statusCode == 405)
    #expect(try decodeBody(ErrorEnvelope.self, from: logsMethod).error.code == "method_not_allowed")

    let missingSession = try api.respond(to: SymphonyAPIRequest(method: "GET", path: "/api/v1/logs/missing-session"))
    #expect(missingSession.statusCode == 404)
    #expect(try decodeBody(ErrorEnvelope.self, from: missingSession).error.code == "session_not_found")
...
@Test func sqliteDiagnosticsCoverPrivateFailureBranches() throws {
    ...
    let unexpectedAutoclosureError = try #require(store.diagnostics.captureRuntimeErrorForUnexpectedAutoclosureProbe())
...
private func decodeBody<T: Decodable>(_ type: T.Type, from response: SymphonyHTTPResponse) throws -> T {
