...
struct OperatorSidebarView: View {
  ...
  private var issueSelection: Binding<IssueID?> {
    Self.issueSelectionBinding(model: model, selectIssue: selectIssue)
  }
...
extension OperatorSidebarView {
  @MainActor
  static func selectIssue(
  ...
  @MainActor
  static func makeSelectIssueAction(
    issueID: IssueID,
    model: SymphonyOperatorModel,
    selectIssue: @escaping (IssueSummary) -> Void
  ) -> () -> Void {
    {
      Self.selectIssue(issueID, model: model, selectIssue: selectIssue)
    }
  }

  @MainActor
  static func issueSelectionBinding(
    model: SymphonyOperatorModel,
    selectIssue: @escaping (IssueSummary) -> Void
  ) -> Binding<IssueID?> {
    Binding(
      ...
      set: { newSelection in
        Self.selectIssue(newSelection, model: model, selectIssue: selectIssue)
      }
    )
  }

  @MainActor
  static func makeServerStatusSummaryView(
    theme: OperatorTheme,
    model: SymphonyOperatorModel,
  ...
  @MainActor
  static func makeIssueSidebarRow(theme: OperatorTheme, issue: IssueSummary, isSelected: Bool)
    -> some View
  {
...
// MARK: - Issue List Subview

private struct IssueSidebarList: View {
  let theme: OperatorTheme
  var model: SymphonyOperatorModel
  @Binding var issueSelection: IssueID?
  let selectIssue: (IssueSummary) -> Void

  var body: some View {
    let list = List(selection: $issueSelection) {
      ForEach(model.filteredIssues, id: \.issueID.rawValue) { issue in
        Button(
          action: OperatorSidebarView.makeSelectIssueAction(
