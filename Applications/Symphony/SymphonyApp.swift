...
struct SymphonyApp: App {
    ...
    nonisolated static func resolveClient(arguments: [String]) -> (any SymphonyAPIClientProtocol)? {
        arguments.contains("--ui-testing") ? UITestingSymphonyAPIClient() : nil
    }
    ...
    init() {
        ...
        let client = Self.resolveClient(arguments: ProcessInfo.processInfo.arguments)
        _model = StateObject(wrappedValue: SymphonyOperatorModel(client: client, initialEndpoint: sharedEndpoint))
...
struct UITestingSymphonyAPIClient: SymphonyAPIClientProtocol {
    func health(endpoint: ServerEndpoint) async throws -> HealthResponse {
        HealthResponse(status: "ok", serverTime: "2026-03-24T12:00:00Z", version: "1.0.0", trackerKind: "github")
    }
    ...
    func issueDetail(endpoint: ServerEndpoint, issueID: IssueID) async throws -> IssueDetail {
        ...
        return IssueDetail(issue: issue, latestRun: run, workspacePath: "/tmp/symphony/atjsh_example_1", recentSessions: [session])
    }
